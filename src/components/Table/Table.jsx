...
export const Table = (tableData) => {
    ...
    const sortTableDataByHighestScore = (data) => {
        ...
        return data.sort(compareTotalWins).map((row, index) => {
            ...
            return (
                <tr key={index}>
                    <th className="tt-table__left-side-header" scope="row">{index + 1}</th>
                    <td>
                        <div className="tt-table__cell-wrapper">
                            {computePosition(index) === 'leader' && <span className="tt-table__leader-icon" dangerouslySetInnerHTML={{ __html: leaderIcon }}></span>}
                            {computePosition(index) === 'secondPlace' && <span className="tt-table__leader-icon" dangerouslySetInnerHTML={{ __html: secondPlaceIcon }}></span>}
                            {computePosition(index) === 'thirdPlace' && <span className="tt-table__leader-icon" dangerouslySetInnerHTML={{ __html: thirdPlaceIcon }}></span>}
                            <p>{row.name}</p>
                        </div>
                    </td>
                    <td>{row.totalWon}</td>
    ...
    return (
        ...
        <caption className="tt-table__caption">Scoreboard</caption>
        <thead>
            <tr>
                <td className="tt-table__invisible-cell"></td>
